...
alter table purchase
modify method_of_payment varchar(20) default 'cash'

alter table staff
modify empid integer generated by default as identity(
    start with 1
    increment by 1) primary key

--insert cardnumbers and method of payment using excel

alter table purchase
add card_number number(16) default -1

update purchase
set method_of_payment = 'cash'

alter table purchase
add constraint fk_purchase__method_of_payment
    foreign key (cust_email, method_of_payment, card_number)
    references customer_method_of_payment(cust_email, method_of_payment, card_number)
